...
SELECT `playerStats`.`id`, `playerStats`.`name`, `playerStats`.`age`, `playerStats`.`nationality`, `playerStats`.`team`, `playerStats`.`mascot`, `playerStats`.`position`, `playerStats`.`games_played`, `playerStats`.`goals`, `playerStats`.`assists`, `playerStats`.`points`, `teaminfo`.`id` AS `teaminfo.id`, `teaminfo`.`mascot` AS `teaminfo.mascot`, `teaminfo`.`teamName` AS `teaminfo.teamName`, `teaminfo`.`primaryColor` AS `teaminfo.primaryColor`, `teaminfo`.`secondaryColor` AS `teaminfo.secondaryColor` FROM `playerStats` AS `playerStats` LEFT OUTER JOIN `teaminfos` AS `teaminfo` ON `playerStats`.`mascot` = `teaminfo`.`mascot` WHERE `playerStats`.`mascot` = 'islanders';
